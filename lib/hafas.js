...
import _vbnProfile from 'hafas-client/p/vbn/index.js'
...
const RANDOM_LOCAL_ADDRESSES_PREFIX = process.env.RANDOM_LOCAL_ADDRESSES_PREFIX || null
let ipRangeAgent = null
if (RANDOM_LOCAL_ADDRESSES_PREFIX) {
	const {createIpPoolAgent} = await import('localaddress-agent')
	...
	ipRangeAgent = await createIpPoolAgent(ipRangeRandomly, 'eth0')
}

const userAgents = [
	['App/4.4.6 (iPhone', 'App/4.5.1 (iPhone'],
	['iOS 14.8.1', 'iOS 15.2', 'iOS 15.4.1'],
...
const transformReq = (_, req) => {
	if (ipRangeAgent) req.agent = ipRangeAgent
...
const rawHafas = createHafas(
	withThrottling(vbnProfile, 50, 1000), // 50 req/s
